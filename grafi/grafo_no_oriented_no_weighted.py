...
def main():
    v = int(input("Inserire il numero di nodi")) #chiedo in input il numero di nodi
    dict = creaDictDaNumNodi(v) #dict = il return della funzione creaDictDaNumNodi con parametro il numero di nodi
    stampaDict(dict) #stampo dict
    stampaMatrice(creaMatriceDaDict(dict, v), v) #stampo la matrice trasformando dict in una matrice
    drawGrafo(dict) #mostra il grafico disegnato con la libreria networkx


def creaMatriceDaNumNodi(v): #crea una matrice con v nodi
    matrix = [] #inizializzo una matrice
